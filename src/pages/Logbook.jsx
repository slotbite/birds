...
const SetViewOnUserLocation = ({ userLocation }) => {
    const map = useMap();
...
// Component to center map on user location (called from button)
const CenterMapOnUser = ({ userLocation, onCenter }) => {
    const map = useMap();
    
    useEffect(() => {
        if (onCenter && userLocation && userLocation.length === 2) {
            map.setView(userLocation, 13, {
                animate: true,
                duration: 1.5
            });
        }
    }, [onCenter, userLocation, map]);
...
const Logbook = () => {
    ...
    const [centerOnUser, setCenterOnUser] = useState(0); // Counter to trigger center
            ...
            if ('geolocation' in navigator) {
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        ...
                        console.log('Location obtained:', userPos, 'Accuracy:', position.coords.accuracy, 'm');
                    ...
                    (error) => {
                        ...
                        // More helpful error message for desktop
                        const errorMessages = {
                            1: 'Permiso denegado. Por favor, permite el acceso a tu ubicación en la configuración del navegador.',
                            2: 'No se pudo determinar tu ubicación. En notebooks, esto puede deberse a que no hay GPS disponible.',
                            3: 'Tiempo de espera agotado. Intenta de nuevo o haz clic en el mapa para establecer tu ubicación manualmente.'
                        };
                        
                        const message = errorMessages[error.code] || 'No se pudo obtener tu ubicación. Haz clic en el mapa para establecerla manualmente.';
                        alert(message);
                    },
                    {
                        enableHighAccuracy: false, // Better for desktop/notebook
                        timeout: 15000,
                        maximumAge: 300000
                    }
                );
                ...
                setLoadingLocation(false);
                alert('Tu navegador no soporta geolocalización. Haz clic en el mapa para establecer tu ubicación.');
            }
        }
    };

    // Function to set location by clicking on map
    const MapClickHandler = () => {
        const map = useMapEvents({
            click(e) {
                const clickedPos = [e.latlng.lat, e.latlng.lng];
                setUserLocation(clickedPos);
                setMapCenter(clickedPos);
                setCenterOnUser(prev => prev + 1);
            }
        });
        return null;
    ...
    useEffect(() => {
        ...
        // Get user's current location (for desktop/notebook, this uses IP-based location)
        if ('geolocation' in navigator) {
            const geoOptions = {
                enableHighAccuracy: false, // Set to false for desktop - uses IP/WiFi instead of GPS
                timeout: 15000, // Longer timeout for desktop
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    console.error('Could not get location:', error);
                    // Don't show error immediately - user can click the button to try again
                    setLoadingLocation(false);
                },
                geoOptions
            );
    ...
    return (
        <div className="h-screen w-full relative flex">
            ...
            <div className="flex-1 relative">
                <MapContainer
                    center={mapCenter}
                    zoom={userLocation ? 13 : (sightings.length > 0 ? 10 : 5)}
                ...
                >
                    ...
                    <CenterMapOnUser userLocation={userLocation} onCenter={centerOnUser} />
                    <MapClickHandler />
                ...
                <button
                    ...
                    className="absolute top-6 right-6 z-10 bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center"
                    title="Centrar en mi ubicación (o haz clic en el mapa para establecerla)"
                >
                    {loadingLocation ? (
